...
RXEQ <- function(t, y, pars) {
  with (as.list(c(y, pars)),{
    ...
    dMIC_A = (CUE[5] * SOMmin[3]) - (MICtrn[[7]]) 
...
# Initialize chain
num_pars <- 6
jump_params <- c(0.01, 0.01, .005, 0.3, 0.3, 0.00005)

# Select parameters for fit
fit_params <- array(NA, dim = c(num_pars, num_iter))
fit_params[1, 1] <- params$Vslope[7]
fit_params[2, 1] <- params$Kslope[7]
fit_params[3, 1] <- params$tau_a[2]
fit_params[4, 1] <- params$Vint[7]
fit_params[5, 1] <- params$Kint[7]
fit_params[6, 1] <- params$tau_a[1]
...
obs_long <- obs_long |>
  mutate(sds = case_when(variable == 'MICa' ~ 0.1,
                         variable == 'SOC' ~ 1))
```
...
for(iter in 2:num_iter){
  ...
  for(j in 1:num_pars){
    ...
    params$Vint[7]  <- proposed_pars[4]
    params$Kint[7]  <- proposed_pars[5]
    params$tau_a[1] <- proposed_pars[6]
...
parameter_MCMC <- tibble(iter = nburn:num_iter,
            ...
            Kslope_MCMC = fit_params[2, nburn:num_iter],
            tauA2_MCMC = fit_params[3, nburn:num_iter],
            Vint_MCMC = fit_params[4, nburn:num_iter],
            Kint_MCMC = fit_params[5, nburn:num_iter],
            tauA1_MCMC = fit_params[6, nburn:num_iter])%>%
