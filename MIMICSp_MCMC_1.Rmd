...
RXEQ <- function(t, y, pars) {
  with (as.list(c(y, pars)),{
    ...
    # ODE System 
    dLIT_1 = I[1]*(1-FI[1]) - LITmin[1] - LITmin[3] - LITmin[5]
    ...
    dMIC_A = CUE[5] * (LITmin[5] + LITmin[6]) + CUE[5] * SOMmin[3] - (MICtrn[[7]] + MICtrn[[8]] + MICtrn[9]) 
...
# Initialize chain
num_pars <- 6
jump_params <- c(0.001, 0.001, .001, 0.001, 0.001, 0.001)

# Select parameters for fit
fit_params <- array(NA, dim = c(num_pars, num_iter))
fit_params[1, 1] <- params$Vslope[7]
fit_params[2, 1] <- params$fPHYS_a[1]
fit_params[3, 1] <- params$Kslope[7]
fit_params[4, 1] <- params$fPHYS_a[2]
fit_params[5, 1] <- params$fCHEM_a[1]
fit_params[6, 1] <- params$tau_a[2]

# Distribution of priors
prior_mean <- c(params$Vslope[7], 
                params$fPHYS_a[1],
                params$Kslope[7],
                params$fPHYS_a[2],
                params$fCHEM_a[1],
...
for(iter in 2:num_iter){
  ...
  for(j in 1:num_pars){
    ...
    log_prior <- dnorm(proposed_pars[1], mean = prior_mean[1], sd = prior_sd[1], log = TRUE) + 
      ...
      dnorm(proposed_pars[4], mean = prior_mean[4], sd = prior_sd[4], log = TRUE) + 
      dnorm(proposed_pars[5], mean = prior_mean[5], sd = prior_sd[5], log = TRUE) + 
      dnorm(proposed_pars[6], mean = prior_mean[6], sd = prior_sd[6], log = TRUE)
    
    params$Vslope[7]  <- proposed_pars[1]
    params$fPHYS_a[1]  <- proposed_pars[2]
    params$Kslope[7] <- proposed_pars[3]
    params$fPHYS_a[2]  <- proposed_pars[4]
    params$fCHEM_a[1]  <- proposed_pars[5]
    params$tau_a[2] <- proposed_pars[6]
    
    # Run Model w/in MCMC
    data <- data <- read.csv("testData.csv", as.is=T)
 
    MIMout_single <- MIMICS1(data[1,], params)
    ...
    MIMrun <- MIMrun |>
      select('FID', 'MICa', 'SOMp', 'SOMc', 'SOMa')
    
    MIMrun_long <- pivot_longer(MIMrun,
                                cols = c('MICa', 'SOMp', 'SOMc', 'SOMa'),
                                names_to = 'variable',
                                values_to = 'predicted')
...
parameter_MCMC <- tibble(iter = nburn:num_iter,
            Vslope_MCMC = fit_params[1, nburn:num_iter],
            fPHYS1_MCMC = fit_params[2, nburn:num_iter],
            Kslope_MCMC = fit_params[3, nburn:num_iter],
            fPHYS2_MCMC = fit_params[4, nburn:num_iter],
            fCHEM1_MCMC = fit_params[5, nburn:num_iter],
            tau2_MCMC = fit_params[6, nburn:num_iter])%>%
